...
def main(args):
    try:
        ...
        b_tree.write(format=1, outfile="subtree_b.newick")

        # split alignment into two sub-alignments
        a_alignment, b_alignment = write_alignment_partitions(alignment, a_tree, b_tree)
        AlignIO.write(a_alignment, "subtree_a.aln", "fasta")
        AlignIO.write(b_alignment, "subtree_b.aln", "fasta")
        ...
        logging.info('Done with the two subtrees')#, flush=True
        # print('Done with first two trees', flush=True)

        # check if the new trees generated by iqtree have the same topology
        a_tree = conform_iqtree_tree("subtree_a.treefile", a_tree.get_children()[0].get_leaf_names())
        b_tree = conform_iqtree_tree("subtree_b.treefile", b_tree.get_children()[0].get_leaf_names())
        ...
        logging.debug(f'branch a: {type(a_branch)} {a_branch}, branch b: {b_branch}')#, flush=True
...
def generate_summary(summary_filename, tree_count, model, increment, taxa_groups, keep):
