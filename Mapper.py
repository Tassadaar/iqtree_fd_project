...
def main(args):
    try:
        ...
        # NOTE: model and nexus_file should be mutually exclusive
        if not nexus_address:
            model_flag = ["-m", f"LG+{model}+G"]
        else:
            model_flag = ["-m", model, "--mdef", nexus_address]
        ...
        for subset in ["test_a", "test_b"]:
            ...
            iqtree_command = iqtree_command + model_flag
        ...
        # else, if it is provided on command line,
            ...
            nexus_address = update_nexus_file(avg_mixture_weights, nexus_address)
...
def update_nexus_file(weights, nexus_address):
    out_freqs = []

    # generate frequency section
    with open(nexus_address, "r") as models:
        section_found = False
        ...
        weight_line = f"model fundi_{model_name} = FMIX{{{','.join(weight_statements)}}};"
...
if __name__ == "__main__":
    ...
    # TODO: user decides if they want to use LG
