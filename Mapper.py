...
def main(args):
    try:
        ...
        # get model, nexus file location and number of cores
        model = args.mixture_model
        nexus_address = args.nexus
        cores = args.cores

        # split master tree into two subtrees based on definition file
        ...
        # run iqtree to re-optimize
        # mixture weights, alpha, branch lengths
        ...
        # iqtree usually outputs unrooted trees
        # conform_iqtree_tree() reroots the out treefile so it rooted the same way as the reference subtree
        ...
        # stitch re-optimized trees back together with the 'fundi' branch
        # the fundi branch connects both subtrees
        # each newly created tree connects both subtrees by attaching the fundi branch
        # on a slightly different spot along the top-most branch of both subtrees
        trees = []
        ...
        # calculate how much each of both subtree re-optimized
        # parameters should weigh in calculating new average mixture weights and alpha
        # weight = taxa in subtree / taxa in master tree
        a_weight, b_weight = calculate_weights(a_tree, b_tree)

        # calculate new alpha (weighted average of re-optimized alphas)
        avg_alpha = calculate_weighted_average_alpha("test_a.iqtree", "test_b.iqtree", a_weight, b_weight)

        # calculate new mixture weights (weighted average of re-optimized mixture weights)
        ...
        # run iqtree on the whole alignment
        #   with the new alpha and new mixture weights
        #   with the fundi branch defined
        #   for each of the new stitched-together trees
        #   calculates per tree: rho, central branch length and log-likelihood
